...
import 'zhihu_question_page.dart' show ZhihuAuthorAvatar, ZhihuMetricRow, zhihuOpenInBrowser;
...
class ZhihuFeedDetailPage extends StatelessWidget {
  const ZhihuFeedDetailPage({super.key, required this.item});
  ...
  String get _fallbackTitle =>
      item.type == 'pin' ? '想法' : (item.authorName.isEmpty ? '详情' : item.authorName);
          ...
          Wrap(
            ...
            children: [
              ...
              if (item.voteupCount > 0)
                ZhihuMetricRow(icon: Icons.thumb_up_outlined, label: '${item.voteupCount} 赞同'),
              if (item.commentCount > 0)
                ZhihuMetricRow(icon: Icons.mode_comment_outlined, label: '${item.commentCount} 评论'),
            ],
          ),
        ],
      ),
