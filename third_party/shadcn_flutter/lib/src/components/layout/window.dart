...
class _WindowLayerGroup extends StatelessWidget {
  /// The effective always-on-top flag for a window, coalescing its explicit
  /// property, its controller state, and a `false` default.
  ///
  /// `Window.controlled(...)` leaves `alwaysOnTop == null` while the window
  /// still carries a controller whose state owns the real value. Comparing the
  /// raw (nullable) flag against the layer's non-null `alwaysOnTop` then always
  /// mismatches, which hid dragged windows from the tree mid-gesture.
  static bool? _isAlwaysOnTop(Window window) {
    return window.alwaysOnTop ??
        window.controller?.value.alwaysOnTop ??
        false;
  }
