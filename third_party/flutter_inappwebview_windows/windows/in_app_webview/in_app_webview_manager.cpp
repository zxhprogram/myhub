...
namespace flutter_inappwebview_plugin
{
  ...
  {
    if (!rohelper_) {
      // Raw pointers on purpose. See the note in in_app_webview_manager.h:
      // these must never be deleted/released so their destructors cannot run
      // during DLL_PROCESS_DETACH (upstream issue #2733).
      ...
      if (rohelper_->WinRtAvailable()) {
        ...
        compositor_ = graphics_context_->CreateCompositor().detach();
