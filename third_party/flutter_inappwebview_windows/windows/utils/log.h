...
namespace flutter_inappwebview_plugin
{
  ...
  {
#ifndef NDEBUG
    // Local patch: "dealloc ..." lines are routine teardown diagnostics that
    // flood the console whenever a WebView or the plugin itself is destroyed
    // (e.g. closing the Google News window). They carry no actionable info,
    // so drop them while keeping real errors visible.
    if (msg.rfind("dealloc ", 0) == 0) {
      return;
    }
